...
- 🔴 **KHÔNG tự format/chỉnh sửa code cũ:** Chỉ sửa đúng phần code liên quan đến yêu cầu. KHÔNG thay đổi alignment, spacing, indentation, hay formatting của code hiện tại mà mình không cần sửa. Ví dụ: nếu thêm 1 dòng mới vào array, KHÔNG chỉnh lại alignment các dòng cũ.
...
### 🧠 MCP Sequential Thinking (Bắt Buộc Toàn Cục)

> 🔴 **BẮT BUỘC:** TRƯỚC KHI thực hiện BẤT KỲ hành động nào, PHẢI sử dụng `mcp_sequential-thinking_sequentialthinking` để phân tích và lập kế hoạch.
> Giao thức đầy đủ trong `@[rules/sequential-thinking]`.

**Tóm tắt nhanh:**

- Mọi yêu cầu (trừ xã giao) → Gọi Sequential Thinking trước
- Tính năng mới: 5-8 bước | Debug: 3-5 bước | Câu hỏi: 2-3 bước
- KHÔNG code, sửa file, chạy lệnh mà chưa qua Sequential Thinking

### 🔧 MCP Serena Code Intelligence (Khi Làm Việc Với Code)

> 🔵 **ƯU TIÊN:** Khi yêu cầu liên quan đến code (edit, debug, enhance, review, brainstorm code), PHẢI kích hoạt Serena và ưu tiên dùng semantic tools.
> Giao thức đầy đủ trong `@[rules/serena-mcp]`.

**Tóm tắt nhanh:**

- Kích hoạt `mcp_serena_activate_project` khi bắt đầu phiên code
- Ưu tiên `find_symbol` thay grep, `get_symbols_overview` thay view_file toàn bộ
- Dùng `find_referencing_symbols` để tìm nơi gọi method
- Kết hợp: Sequential Thinking (phân tích) + Serena (code) + MCP DB (data)
